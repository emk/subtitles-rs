...
pub fn export_csv(exporter: &mut Exporter) -> Result<()> {
    ...
    let mut buffer = Vec::<u8>::new();
    {
        ...
        let aligned: Vec<(Option<Subtitle>, Option<Subtitle>)> =
            exporter.align().iter()
        ...
        for ctx in aligned.items_in_context() {
            ...
            if let Some(curr) = foreign.curr {
                ...
                let sort_key = format!("{}{}", &prefix,
                                       &seconds_to_hhmmss_sss(period.begin()));
                ...
                try!(wtr.serialize(&note));
            }
        }
    }

    // Write out our CSV file.
    try!(exporter.export_data_file("cards.csv", &buffer));
